...
export function createFilterControls(container, synthEngine) {
  ...
  cutoffLabel.textContent = ' filter freq ';  // Updated label
  const cutoffSlider = document.createElement('input');
  cutoffSlider.type = 'range';
  cutoffSlider.min = 0;
  cutoffSlider.max = 100;
  cutoffSlider.value = 100;  // default
  cutoffSlider.step = 0.5;  // Finer control

  cutoffSlider.addEventListener('input', e => {
    const linearValue = parseFloat(e.target.value);
    // More exponential curve for better control in lower frequencies
    const minFreq = 80;
    const maxFreq = 12000;
    const exp = 3; // Adjust curve steepness
    const normalizedValue = Math.pow(linearValue / 100, exp);
    const freqValue = minFreq * Math.pow(maxFreq / minFreq, normalizedValue);
    synthEngine.setCutoffFrequency(freqValue);
  });
